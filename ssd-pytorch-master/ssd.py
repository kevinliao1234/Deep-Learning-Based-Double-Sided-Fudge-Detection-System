...
class SSD(object):
    ...
    def detect_image_raw(self, image):
        # 1. 預處理圖片（轉RGB、resize、歸一化、增加batch維度）
        image_shape = np.array(np.shape(image)[0:2])
        image = cvtColor(image)
        image_data = resize_image(image, (self.input_shape[1], self.input_shape[0]), self.letterbox_image)
        image_data = np.expand_dims(np.transpose(preprocess_input(np.array(image_data, dtype='float32')), (2, 0, 1)), 0)
        ...
        with torch.no_grad():
            ...
            # 2. 模型原生預測結果 (還沒解碼)
            outputs = self.net(photo)

            # print("[DEBUG] 模型原生輸出 tensors 數量:", len(outputs))
            # for i, out in enumerate(outputs):
            #     print(f"[DEBUG] outputs[{i}] shape: {out.shape}, sample data: {out[0][0][:5]}")
            ...
            # print(f"[DEBUG] 解碼後結果數量: {len(results)}")
            ...
            top_label = res[:, 4].astype(int)
            boxes = res[:, :4].astype(int).tolist()
            scores = res[:, 5].astype(float).tolist()


            # print(f"[DEBUG] 偵測到的物件數量: {len(top_label)}")
            # print(f"[DEBUG] 偵測標籤: {top_label}")
            # print(f"[DEBUG] 偵測分數: {top_conf}")

            return boxes, top_label, scores
